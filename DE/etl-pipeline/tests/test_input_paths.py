...
def test_all_retailer_patterns_accept_showcase_and_future_sample_names(
    tmp_path: Path,
) -> None:
    filenames = {
    ...
    for model, filename in filenames.items():
        assert resolve_input_paths(
            None, config, model, "products", date(2026, 8, 14)
        ) == [str(tmp_path / model.removeprefix("products_") / filename)]


def test_products_glob_is_used_when_the_exact_template_does_not_exist(
    tmp_path: Path,
) -> None:
    ...
    assert resolve_input_paths(
        None, config, "products_aldi", "products", date(2026, 5, 4)
    ) == [str(expected)]
