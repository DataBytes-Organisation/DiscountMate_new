...
USING (
    ...
    physical_key_identity AS (
        SELECT
            raw.source_product_key,
            products.id AS product_id
        FROM raw_input_normalized AS raw
        INNER JOIN {{ dim_products_table }} AS products
            ON coalesce(lower(products.brand_name), '') = coalesce(lower(raw.brand_name), '')
            AND lower(products.product_name) = lower(raw.standardized_product_name)
            AND coalesce(products.pack_quantity, -1) = coalesce(raw.pack_quantity, -1)
            AND coalesce(lower(products.pack_uom), '') = coalesce(lower(raw.pack_uom), '')
    ),
    resolved_canonical_groups AS (
        SELECT
            source_rows.canonical_key,
            coalesce(
                min(physical_key_identity.product_id) FILTER (WHERE physical_key_identity.product_id IS NOT NULL),
        ...
        LEFT JOIN canonical_identity
            USING (source_product_key)
        LEFT JOIN physical_key_identity
            USING (source_product_key)
    ...
    product_identity AS (
        SELECT
            source_rows.source_product_key,
            coalesce(
                physical_key_identity.product_id,
