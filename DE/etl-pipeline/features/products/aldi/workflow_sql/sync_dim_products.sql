...
USING (
    ...
    pending_observations AS (
        ...
        INNER JOIN canonical_products
            USING (source_product_key)
        CROSS JOIN retailer_aldi AS retailer
    ),
    ...
    price_snapshots AS (
        SELECT
            ...
            max(price) FILTER (WHERE price_rank = 1) AS price_current_aldi,
            max(price) FILTER (WHERE price_rank = 2) AS price_last_aldi,
            max(unit_price) FILTER (WHERE price_rank = 1) AS unit_price_current_aldi,
            max(unit_price) FILTER (WHERE price_rank = 2) AS unit_price_last_aldi
        FROM ranked_prices
    ...
    merged_source AS (
        SELECT
            ...
            price_snapshots.price_current_aldi,
            price_snapshots.price_last_aldi,
            price_snapshots.unit_price_current_aldi,
            price_snapshots.unit_price_last_aldi,
...
WHEN MATCHED AND (
    ...
    OR target.price_current_aldi IS DISTINCT FROM source.price_current_aldi
    OR target.price_last_aldi IS DISTINCT FROM source.price_last_aldi
    OR target.unit_price_current_aldi IS DISTINCT FROM source.unit_price_current_aldi
    OR target.unit_price_last_aldi IS DISTINCT FROM source.unit_price_last_aldi
    ...
    price_current_aldi = source.price_current_aldi,
    price_last_aldi = source.price_last_aldi,
    unit_price_current_aldi = source.unit_price_current_aldi,
    unit_price_last_aldi = source.unit_price_last_aldi,
...
WHEN NOT MATCHED THEN INSERT (
    ...
    source.pack_uom,
    source.price_current_aldi,
    source.price_last_aldi,
    source.unit_price_current_aldi,
    source.unit_price_last_aldi,
