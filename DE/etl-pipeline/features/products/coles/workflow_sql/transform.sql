...
WITH cleaned AS (
    SELECT
        ...
        TRY_CAST(
            regexp_replace(
                coalesce(CAST(Price_Now AS VARCHAR), ''),
                '[^0-9.-]',
                '',
                'g'
            ) AS DOUBLE
        ...
        TRY_CAST(
            regexp_replace(
                coalesce(CAST(Price_Was AS VARCHAR), ''),
                '[^0-9.-]',
                '',
                'g'
            ) AS DOUBLE
        ...
        TRY_CAST(
            regexp_replace(
                coalesce(CAST(UnitPrice AS VARCHAR), ''),
                '[^0-9.-]',
                '',
                'g'
            ) AS DOUBLE
        ...
        CASE
            WHEN lower(trim(coalesce(CAST(OnlineSpecial AS VARCHAR), '')))
                IN ('true', '1', 'yes', 'y')
                THEN TRUE
...
parsed AS (
    SELECT
        *,
        ...
        CASE
            WHEN COALESCE(price, 0) < COALESCE(price_was, 0)
                THEN TRUE

            WHEN upper(coalesce(comparable, '')) = 'SPECIAL'
                THEN TRUE

            WHEN online_special IS TRUE
                THEN TRUE
...
categorized AS (
    SELECT
        *,
        ...
        CASE
            WHEN online_subcategory_lower IN ('back to school', 'easter')
                THEN 'SEASONAL'

            WHEN online_category_lower LIKE '%easter%'
                THEN 'SEASONAL'
            ...
            WHEN online_category_lower = 'international foods'
                THEN 'CONTINENTAL'

            WHEN online_category_lower IN (
                'ready to eat meals',
                'frozen meals'
            )
                THEN 'CONVENIENCE FOOD'

            WHEN online_category_lower LIKE '%health foods%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%sports nutrition%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%vitamin%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%supplement%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%diet%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%vegan%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_category_lower LIKE '%vegetarian%'
                THEN 'HEALTH FOOD & SUPPLEMENTS'

            WHEN online_subcategory_lower = 'liquorland'
                THEN 'ALCOHOL'

            WHEN online_subcategory_lower = 'baby'
                THEN 'BABY FOOD & ACCESSORIES'

            WHEN online_subcategory_lower = 'bakery'
                THEN 'BAKERY'

            WHEN online_subcategory_lower = 'drinks'
                THEN 'BEVERAGES'

            WHEN online_subcategory_lower = 'pantry'
                THEN 'PANTRY'

            WHEN online_subcategory_lower = 'chips, chocolates & snacks'
                THEN 'SNACKS & CONFECTIONARY'

            WHEN online_subcategory_lower = 'dairy, eggs & fridge'
                THEN 'DAIRY & REFRIGERATED'

            WHEN online_subcategory_lower = 'deli'
                THEN 'DELI & CHILLED MEALS'

            WHEN online_subcategory_lower = 'frozen'
                THEN 'FROZEN FOODS'

            WHEN online_subcategory_lower = 'fruit & vegetables'
                THEN 'FRUIT, VEG & PRODUCE'

            WHEN online_subcategory_lower = 'health & beauty'
                THEN 'HEALTH & BEAUTY'
            ...
            WHEN online_subcategory_lower = 'pet'
                THEN 'PET FOOD & ACCESSORIES'

            WHEN online_subcategory_lower = 'meat & seafood'
                THEN 'POULTRY, MEAT & SEAFOOD'

            WHEN online_category_lower LIKE '%deli%'
                THEN 'DELI & CHILLED MEALS'
