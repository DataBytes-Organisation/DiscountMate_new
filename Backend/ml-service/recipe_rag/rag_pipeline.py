...
class MongoProductResolver:
    ...
    def compare_product_prices(
        ...
        """
        Compare the latest prices for one matched product across retailers.

        All product and price information comes from MongoDB.
        No prices are generated by the LLM.
        """
        if not self.enabled:
            return {
                "status": "unavailable",
                "prices": [],
                "cheapest": None,
                "error": "MongoDB product service is unavailable",
            }
        ...
        if not candidates:
            return {
                ...
                "prices": [],
                "cheapest": None,
                "error": "No matching product was found",
            }
        ...
        try:
            # ---------------------------------------------------------
            # 1. Resolve semantic candidates to real MongoDB products
            # ---------------------------------------------------------
            products = self.resolve_names(candidates)
            ...
            if not products:
                return {
                    ...
                    "prices": [],
                    "cheapest": None,
                    "error": "No matching product was found in MongoDB",
                }

            # For V1 use the strongest semantic candidate.
            selected = products[0]

            lookup = selected.get("lookup", {})
            product_code = lookup.get("product_code")
            product_id = lookup.get("product_id")
            mongo_id = lookup.get("_id")
            ...
            if product_code not in (None, ""):
                pricing_filters.append({
                    "product_code": {
                        "$in": self._value_variants([str(product_code)])
                    }
                })
            ...
            if product_id not in (None, ""):
                pricing_filters.append({
                    "product_id": {
                        "$in": self._value_variants([str(product_id)])
                    }
                })
            ...
            if mongo_id not in (None, ""):
                pricing_filters.append({
                    "product_id": {
                        "$in": self._value_variants([str(mongo_id)])
                    }
                })

            if not pricing_filters:
                return {
                    "status": "not_found",
                    "prices": [],
                    "cheapest": None,
                    "error": "Matched product has no pricing identifier",
                }
            ...
            # ---------------------------------------------------------
            # 3. Retrieve latest pricing records
            # ---------------------------------------------------------
            pricings = list(
                self._pricings_col.find(
                    pricing_query,
                    {
                        "product_code": 1,
                        "product_id": 1,
            ...
            # ---------------------------------------------------------
            # 4. Keep only the latest valid price per retailer
            # ---------------------------------------------------------
            latest_by_store = {}

            for pricing in pricings:
                store = str(
                    pricing.get("store_chain", "")
                ).strip()
                ...
                if store_key not in latest_by_store:
                    latest_by_store[store_key] = {
                        "retailer": store,
                        "price": price,
                        "currency": "AUD",
                    }

            prices = list(latest_by_store.values())
            ...
            # ---------------------------------------------------------
            # 7. Return grounded comparison
            # ---------------------------------------------------------
            return {
                "status": "success" if prices else "no_prices",
                "matched_product": {
                    "product_id": str(
                        lookup.get("product_id")
                        or lookup.get("product_code")
                        or lookup.get("_id")
                        or ""
                    ),
            ...
            print(
                f"[MongoProductResolver] compare_product_prices error: {e}"
            )
...
class RecipeRAG:
    ...
    def __init__(self,
        ...
        # Recipe retrieval is optional so other chatbot features
        # can work even when the recipe index is unavailable.
        try:
            self.retriever = RecipeRetriever(self.model, index_dir)
            print("[RecipeRAG] recipe retriever loaded successfully")
        except Exception as e:
            self.retriever = None
            print(f"[RecipeRAG] WARNING: recipe retriever unavailable: {e}")

        # Product matching is required for price comparison.
    ...
    # -- core query ---------------------------------------------------------
    def _is_price_comparison_query(self, user_query: str) -> bool:
        """Detect simple price-comparison requests."""
        ...
        return (
            any(word in query for word in price_words)
            and any(phrase in query for phrase in comparison_phrases)
        ) or any(
            phrase in query
            for phrase in (
                "where is cheapest",
                "where is the cheapest",
                "which store has the cheapest",
                "compare prices",
                "price comparison",
            )
        )
    ...
    def chat(self, session_id: str, user_query: str, top_k: int = 3) -> Dict:
        ...
        price_tool_selection = {
            "intent": "price_comparison",
            "tools": ["product_lookup"],
            "reason": "message asks for retailer price comparison",
            "confidence": 0.9,
        ...
        # Handle price-comparison requests without invoking recipe retrieval or LLMs.
        if self._is_price_comparison_query(user_query):
            candidates = self.product_matcher.find_query_product_candidates(
                user_query,
                top_k=10,
            )

            comparison = self.mongo_resolver.compare_product_prices(candidates)

            if comparison.get("status") not in ("success", "no_prices"):
                return {
                    "answer": "I couldn't find reliable price information for that product. Please try specifying the product name, brand, or pack size.",
                    "sources": [],
                    "turns": self.turn_count(session_id),
                    "limit_reached": False,
                    "product_candidate_names": [],
                    "products_pending": False,
                    "recipe_context_id": None,
                    "tool_selection": price_tool_selection,
                }

            matched_product = comparison.get("matched_product") or {}
            prices = comparison.get("prices") or []
            cheapest = comparison.get("cheapest")

            product_name = (
                matched_product.get("product_name")
                or matched_product.get("name")
                or matched_product.get("description")
                or "the product"
            )

            if not prices:
                return {
                    "answer": f"I found {product_name}, but there are no current retailer prices available.",
                    "sources": [],
                    "turns": self.turn_count(session_id),
                    "limit_reached": False,
                    "product_candidate_names": [product_name],
                    "products_pending": False,
                    "recipe_context_id": None,
                    "tool_selection": price_tool_selection,
                }
            ...
            for item in prices:
                retailer = item.get("store_chain") or item.get("retailer") or "Unknown retailer"
                price = item.get("price")

                if price is not None:
                    lines.append(f"- {retailer}: ${float(price):.2f}")

            if cheapest:
                cheapest_store = (
                    cheapest.get("store_chain")
                    or cheapest.get("retailer")
                    or "Unknown retailer"
                )
                ...
                if cheapest_price is not None:
                    lines.append(
                        f"\nCheapest option: {cheapest_store} "
                        f"at ${float(cheapest_price):.2f}."
                    )
        ...
        if selection.reuse_previous_results and previous_results:
            ...
            if self.retriever is None:
                return {
                    "answer": (
                        "Recipe search is temporarily unavailable, but I can still "
                        "help compare grocery prices if you ask about a specific product."
                    ),
                    "sources": [],
                    "turns": self.turn_count(session_id),
                    "limit_reached": False,
