# Coles Synthetic Customer Transaction Data Generator
# This script generates 40,000 synthetic customer transactions based on the Coles product catalog
...
def read_product_catalog(filename):
    try:
        # Read the CSV file - Coles data already has numeric price columns
...
def get_random_preferred_categories():
    # Get a set of unique categories from the Coles data
    # This will be populated dynamically based on the actual Coles categories
    all_categories = [
        "Pantry", "Dairy, Eggs & Chilled", "Fresh Produce", "Bakery",
        "Meat & Seafood", "Deli", "Cleaning & Laundry",
        "Health & Beauty", "Household", "Snacks & Confectionery",
        "Drinks", "Liquor", "International Foods", "Freezer"
    ]
    ...
    return [
        {
            ...
            "categories": ["Pantry", "Dairy, Eggs & Chilled", "Fresh Produce", "Bakery", "Meat & Seafood"],
        ...
        },
        {
            ...
            "frequency": 0.2,
            "categories": ["Deli", "Bakery", "Dairy, Eggs & Chilled", "Drinks"],
        ...
        },
        {
            ...
            "frequency": 0.15,
            "categories": ["Fresh Produce", "Dairy, Eggs & Chilled", "Meat & Seafood"],
        ...
        },
        {
            "name": "Party Supplies",
            "frequency": 0.05,
            "categories": ["Snacks & Confectionery", "Drinks", "Liquor"],
        ...
        },
        {
            ...
            "frequency": 0.15,
            "categories": ["Cleaning & Laundry", "Health & Beauty", "Household"],
        ...
        },
        {
            ...
            "frequency": 0.1,
            "categories": ["International Foods", "Pantry", "Meat & Seafood"],
            "itemCount": {"min": 5, "max": 12},
            "regularItems": 0.4,
        }
    ]
...
def generate_transaction(customer_id, transaction_date, products, products_by_category, patterns, customer_personas,
    ...
    # Update persona preferred categories to match actual categories if needed
    for i in range(len(persona['preferredCategories'])):
        if persona['preferredCategories'][i] not in actual_categories:
            persona['preferredCategories'][i] = random.choice(actual_categories)
    ...
    # Map selected pattern categories to actual available categories
    mapped_categories = []
    for pattern_category in selected_pattern['categories']:
        if pattern_category in actual_categories:
            mapped_categories.append(pattern_category)
        else:
            # Find similar categories or just pick a random one
            mapped_categories.append(random.choice(actual_categories))

    # Update pattern with mapped categories
    selected_pattern['categories'] = mapped_categories
    ...
    for category in selected_pattern['categories']:
        ...
        if category in products_by_category and len(products_by_category[category]) > 0:
            ...
            purchased_items.append({
                ...
                'itemName': selected_product['item_name'],
                'price': selected_product['item_price'] or 0,
    ...
    while len(purchased_items) < item_count:
        ...
        if category_products:
            ...
            existing_product = next(
                (item for item in purchased_items if item['productCode'] == selected_product['product_code']),
                None
            )
...
def generate_transaction_data(filename, num_transactions=40000):
    ...
    # Get actual categories from the Coles data
    actual_categories = list(products_by_category.keys())
    ...
    output_file = 'coles_customer_transactions.csv'
...
# Example usage
def main():
    print("Generating Coles synthetic transaction data...")

    result = generate_transaction_data('Coles_cleaned.csv', 40000)
