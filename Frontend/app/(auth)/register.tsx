...
function Input({ label, placeholder, icon, secure }: InputProps) {
   return (
      <View className="gap-1">
         ...
         <View className="flex-row items-center gap-2 border border-gray-200 rounded-xl px-3 h-11 bg-gray-50">
            {icon ? (
...
function StatCard({ label, value }: StatProps) {
   return (
      <View className="flex-1 rounded-2xl border border-emerald-100 bg-emerald-50/70 px-4 py-3">
         ...
         <Text className="mt-1 text-[11px] text-emerald-800">{label}</Text>
      </View>
   );
}
      ...
      <View className="flex-1 rounded-2xl border border-gray-100 bg-white px-4 py-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
         <View className="flex-row items-center gap-1 mb-2">
            {Array.from({ length: 5 }).map((_, index) => (
               ...
               <Ionicons key={index} name="star" size={13} color="#F59E0B" />
            ))}
         </View>
         <Text className="text-xs text-gray-700">{quote}</Text>
         <View className="mt-3">
            <Text className="text-xs font-semibold text-gray-900">
               {name}
            </Text>
            <Text className="text-[11px] text-gray-500">{role}</Text>
         </View>
      </View>
   );
}
...
function FAQItem({ question, answer }: FAQProps) {
   return (
      <View className="rounded-2xl border border-gray-100 bg-white px-4 py-3">
         ...
         <Text className="mt-2 text-[11px] text-gray-600">{answer}</Text>
      </View>
   );
}
...
function StepCard({ icon, title, description }: StepProps) {
   return (
      <View className="flex-1 rounded-2xl border border-gray-100 bg-white px-4 py-4">
         ...
         <Text className="mt-1 text-[11px] text-gray-600">{description}</Text>
      </View>
   );
}
...
export default function RegisterScreen() {
   return (
      ...
      >
         {/* Hero */}
         <View className="items-center mb-6 mt-4 px-4">
            ...
            <Text className="mt-2 text-sm text-gray-500 text-center leading-5 max-w-[360px]">
         ...
         <View className="w-full px-4 pb-10 pt-6">
            <View className="w-full max-w-5xl self-center gap-8">
               ...
               <View className="flex-col gap-6 md:flex-row md:items-start">
                  ...
                  <View className="flex-1 rounded-3xl border border-gray-100 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
                     <View className="mb-4">
                        <Text className="text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-600">
                           Create your account
                        </Text>
                     ...
                     <View className="mt-5 gap-3">
                        ...
                        <View className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 gap-2">
                           <Text className="text-[11px] font-medium text-gray-800 mb-1">
                              How would you describe your usual grocery pattern?
                           </Text>

                           <View className="flex-row items-start gap-2">
                              <View className="mt-[3px] h-3.5 w-3.5 items-center justify-center rounded-full border border-emerald-600 bg-emerald-600">
                           ...
                           <View className="flex-row items-start gap-2">
                              <View className="mt-[3px] h-3.5 w-3.5 rounded-full border border-gray-300 bg-white" />
                              <Text className="flex-1 text-[11px] text-gray-700">
                     ...
                     <View className="mt-5 gap-2">
                        <View className="flex-row items-start gap-2">
                           <View className="mt-[2px] h-4 w-4 items-center justify-center rounded-[6px] bg-emerald-600">
                              <Ionicons name="checkmark" size={12} color="#ECFDF3" />
                           </View>
                        ...
                        <View className="flex-row items-start gap-2">
                           <View className="mt-[2px] h-4 w-4 items-center justify-center rounded-[6px] border border-gray-300 bg-white" />
                           <Text className="flex-1 text-[11px] text-gray-600">
                     ...
                     <View className="mt-6 gap-3">
                        <Pressable className="h-11 items-center justify-center rounded-2xl bg-emerald-600">
                  ...
                  <View className="flex-1 gap-5">
                     ...
                     <View className="mt-4 gap-3">
                        <View className="rounded-3xl border border-emerald-100 bg-emerald-50 px-5 py-6">
                           ...
                           <Text className="mt-2 text-[11px] text-emerald-800">
                           ...
                           <View className="mt-4 gap-2">
                              <View className="flex-row items-center gap-2">
                                 <Ionicons
                                    ...
                                    color="#047857"
                                 />
                                 <Text className="text-[11px] text-emerald-900">
                        ...
                        <View className="rounded-2xl border border-gray-100 bg-white px-4 py-4">
                           ...
                           <View className="mt-3 gap-2">
                              <View className="flex-row items-center gap-2">
                                 <View className="h-4 w-4 items-center justify-center rounded-[6px] bg-emerald-600">
                                    <Ionicons name="checkmark" size={12} color="#ECFDF3" />
                                 </View>
               ...
               <View className="mt-10 rounded-3xl bg-slate-900 px-5 py-7">
                  <Text className="text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-400">
                  ...
                  <View className="mt-4 flex-row flex-wrap items-center gap-3">
                     <Pressable className="h-10 items-center justify-center rounded-2xl bg-emerald-500 px-5">
                        <Text className="text-xs font-semibold uppercase tracking-[0.16em] text-white">
                           Create free account
                        </Text>
                     </Pressable>
                     <Pressable className="h-10 items-center justify-center rounded-2xl border border-slate-500 px-5">
