// Frontend/components/home/ProductGrid.tsx
...
function apiProductHasShelfPrice(product: ApiProduct): boolean {
  ...
  return (
    colesPriceNum != null || woolworthsPriceNum != null || igaPriceNum != null
  );
}

/**
 * Note: ALDI has no corresponding field on ApiProduct yet (no aldi_price from the API),
 * so an "aldi" selection here will never match until the backend exposes ALDI pricing.
 * This is a known gap, not a bug in this filter.
 */
function productMatchesRetailers(
  product: ApiProduct,
  retailersFilter?: string[],
): boolean {
  if (!retailersFilter || retailersFilter.length === 0) return true;
  const { colesPriceNum, woolworthsPriceNum, igaPriceNum } =
    resolveColesWoolworthsPrices(product);
  return retailersFilter.some((key) => {
    if (key === "coles") return colesPriceNum != null;
    if (key === "woolworths") return woolworthsPriceNum != null;
    if (key === "iga") return igaPriceNum != null;
    return false;
  });
}
...
function parseProductsPayload(
  ...
  if (Array.isArray(data)) {
    ...
    return {
      ...
      totalPages: Math.max(
        1,
        Math.ceil(items.length / Math.max(1, limitForFallback)),
      ),
    };
  }
  const d = data as {
...
export function mapApiProductToCard(product: ApiProduct): Product {
  ...
  const woolworthsUnitPriceLabel = sanitizeUnitLabel(
    product.woolworths_unit_price,
  );
  ...
  const retailers = prices.map((p) => ({
    ...
    isCheapest:
      lowestPrice != null && p.price != null && p.price === lowestPrice,
    unitPriceLabel:
      p.storeKey === "coles"
        ? colesUnitPriceLabel
  ...
  return {
    ...
    packQuantity:
      product.unit_per_prod == null ? undefined : String(product.unit_per_prod),
...
const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  const [totalPagesFromApi, setTotalPagesFromApi] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    run();
    return () => ac.abort();
  }, [currentPage, activeCategory, searchQuery, requireSearch, retryRequest]);
  ...
  const filteredApiProducts: ApiProduct[] = apiProducts.filter((product) => {
    ...
    if (!productMatchesRetailers(product, retailersFilter)) {
      return false;
    }
  ...
  const apiMappedProducts: Product[] =
    filteredApiProducts.map(mapApiProductToCard);
    ...
    <>
      ...
      {error && !loading && (
        <View className="mb-6 px-6 py-8 rounded-2xl bg-red-50 border border-red-200 items-center">
          ...
          <Text className="text-sm text-red-600 text-center mb-5">{error}</Text>
            ...
            <View className="mt-8 flex-row items-center justify-center space-x-2">
              ...
              <Pressable
                className={`px-3 py-2 rounded-xl border-2 ${
                  canGoPrev ? "border-gray-200" : "border-gray-100 bg-gray-50"
                }`}
                disabled={!canGoPrev}
                onPress={() =>
              ...
              >
                <Text
                  className={`text-sm ${
                    canGoPrev ? "text-emerald-500" : "text-gray-300"
                  }`}
                >
                  {"≪"}
                </Text>
              ...
              <Pressable
                className={`px-3 py-2 rounded-xl border-2 ${
                  canGoNext ? "border-gray-200" : "border-gray-100 bg-gray-50"
                }`}
                disabled={!canGoNext}
                onPress={() =>
              ...
              >
                <Text
                  className={`text-sm ${
                    canGoNext ? "text-emerald-500" : "text-gray-300"
                  }`}
                >
                  {">"}
                </Text>
        ...
        <View className="flex-row items-start gap-4 mb-4">
          ...
          <View className="flex-1 min-w-0">
            <View
              ...
              style={{ width: "80%" } as any}
            />
            <View
              className="h-3 bg-gray-100 rounded mb-3"
              style={{ width: "60%" } as any}
            />
            <View className="flex-row flex-wrap items-center gap-2">
              <View
                className="h-5 bg-gray-200 rounded-full"
                style={{ width: 70 } as any}
              />
              <View
        ...
        <View className="border-t border-gray-100 pt-4 mb-4">
          <View className="flex-row justify-between gap-3">
            {[1, 2, 3].map((i) => (
              <View
                key={i}
                className="flex-1 p-2 rounded-lg bg-gray-50 items-center"
              >
                <View
                ...
                />
                <View
                  className="h-4 bg-gray-300 rounded mb-1"
                  style={{ width: "50%" } as any}
                />
                <View
                  className="h-2 bg-gray-100 rounded"
                  style={{ width: "40%" } as any}
                />
              </View>
            ))}
          </View>
